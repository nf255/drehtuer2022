    ...
    if c_out["error"] == "none":
        while True:
            ...
            if e_flag == (e_flag | 2 ** 2):
                extended_result["time"] = int(c_data[1], base=16) + (int(c_data[2], base=16) << 4) + (
                            int(c_data[3], base=16) << 8) + (int(c_data[4], base=16) << 12)
                del c_data[1:5]
            if e_flag == (e_flag | 2 ** 3):
                extended_result["pc"] = int(c_data[1], base=16) + (int(c_data[2], base=16) << 4) + (
                            int(c_data[3], base=16) << 8) + (int(c_data[4], base=16) << 12) + (
                                                    int(c_data[5], base=16) << 16) + (int(c_data[6], base=16) << 20)
                del c_data[1:7]
            if e_flag == (e_flag | 2 ** 4):
                extended_result["frequency"] = int(c_data[1], base=16) + (int(c_data[2], base=16) << 4) + (
                            int(c_data[3], base=16) << 8)
                del c_data[1:4]
...
button_SyncGetEPCs = Button(root, text="SyncGetEPCs", command=SyncGetEPCs)
